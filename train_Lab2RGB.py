...
def main():
    ...
    train_dataset = DataSet(
        "Lab2RGB", args.iteration * args.batchsize, args.width, args.height)
    test_dataset = DataSet("Lab2RGB", 100, args.width, args.height)
    ...
    if args.test:
        ...
        cv2.imwrite(args.out + '/ideal_image.png', img)
        t_img = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
        cv2.imwrite(args.out + '/input_image.png', t_img)

        pred = model.model(xp.array([[np.ndarray.flatten(t_img / 255)]])
